...
async function importDeSoGeodata() {
    ...
    const connected = await (0, database_1.checkConnection)();
    if (!connected) {
    ...
    const existingCount = await (0, database_1.query)('SELECT COUNT(*) FROM deso_areas');
    ...
    let geojson;
    try {
        const response = await axios_1.default.get(SCB_WFS_URL, {
            params,
    ...
    for (let i = 0; i < geojson.features.length; i++) {
        ...
        try {
            ...
            let category = 'C';
            if (props.kategori) {
                category = props.kategori.toUpperCase();
            }
            ...
            await (0, database_1.query)(`INSERT INTO deso_areas (
          deso_code,
          name,
          kommun_code,
          kommun_name,
    ...
    console.log('\n6. Verifying import...');
    const finalCount = await (0, database_1.query)('SELECT COUNT(*) FROM deso_areas');
    ...
    console.log('\n7. Sample DeSO areas:');
    const samples = await (0, database_1.query)(`
